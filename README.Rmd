...
To avoid the latter we could call `library(dm)` before cleaning up (these dependencies would
be copied in dm's imports env so `dm::separate()` would work but `tidyr::separate()` wouldn't), or call `clean_up_tidyverse(force_keep = c("separate", "transmute"))` above to keep the necessary dependencies around.
